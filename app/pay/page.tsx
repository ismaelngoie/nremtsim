...
const PRICING = {
  monthly: { price: 19, cadence: "/mo", strike: null as number | null },
...
export default function PaywallPage() {
  ...
  // Personalization (pull from localStorage if you store these elsewhere)
  ...
  useEffect(() => {
    ...
    const dt = Number(localStorage.getItem("daysToExam"));
    ...
    if (Number.isFinite(dt) && dt >= 0 && dt <= 365) setDaysToExam(Math.round(dt));
  }, []);
  ...
  const checkoutHref = useMemo(() => {
    // Change this to your real checkout route (Stripe, Paddle, etc.)
    return `/dashboard?plan=${selectedPlan}`;
  }, [selectedPlan]);

  return (
    <div className="min-h-screen bg-[#0B1226] text-white font-sans flex flex-col items-center px-4 md:px-6 relative overflow-y-auto">
      {/* Background glows */}
      ...
      <div className="w-full max-w-sm z-10 pt-5 pb-28">
        {/* Top header */}
        <motion.div
        ...
        >
          ...
          <h1 className="mt-3 text-3xl font-black tracking-tight leading-[1.05]">
            Unlock Your <span className="text-cyan-300">Readiness Plan</span>
          </h1>
          ...
          <p className="mt-2 text-slate-300 text-sm leading-relaxed">
            ...
            <span className="font-bold text-red-300">({weakPct}%)</span>.
        ...
        <motion.div
          ...
          className="mb-5"
        >
          <div className="rounded-2xl bg-slate-900/40 border border-white/10 px-4 py-3 flex items-center justify-between gap-3">
        ...
        <div className="mb-6 rounded-2xl bg-slate-900/45 border border-white/10 p-5">
          ...
          <div className="mt-4 space-y-3">
            ...
            <FeatureRow icon="📚" text="Question bank + rapid review drills" />
        ...
        {/* Plan comparison (clean + not “table-y”) */}
        <div className="mb-6 rounded-2xl bg-slate-900/40 border border-white/10 p-5">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-black text-slate-300 uppercase tracking-widest">
              What you unlock today
            </h3>
            <span className="text-[11px] font-mono text-slate-400">All plans include:</span>
          </div>

          <div className="mt-4 grid gap-3">
            <CompareRow label="Unlimited full simulations + timers" />
            <CompareRow label="Full rationales + missed-question review" />
            <CompareRow label="Personalized 14-day weakness plan" />
            <CompareRow label="Readiness score + domain tracking" />
            <CompareRow label="EMT + Paramedic modes" />
          </div>

          <div className="mt-4 rounded-xl bg-white/5 border border-white/10 p-4">
            <div className="text-sm font-extrabold">Why Annual wins</div>
            <div className="mt-1 text-sm text-slate-300 leading-relaxed">
              Most candidates study for weeks. Annual keeps your progress + costs less than 7 months of
              monthly.
            </div>
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20">
        <div className="mx-auto w-full max-w-sm px-4 pb-4">
          <div className="rounded-2xl bg-[#0B1226]/80 backdrop-blur border border-white/10 p-3 shadow-[0_-15px_40px_-20px_rgba(0,0,0,0.8)]">
