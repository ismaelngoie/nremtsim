...
export default function SimulatorPage() {
  ...
  // Logic: Handle "Next" Button
  const handleNext = () => {
    if (selectedOption === null) return;

    // IF WE ARE AT QUESTION 5 (Index 4), TRIGGER THE GLITCH
    if (currentQIndex === 4) {
      triggerGlitchSequence();
    } else {
      // Normal behavior: Go to next question
      setCurrentQIndex((prev) => prev + 1);
      setSelectedOption(null);
    }
  };

  // The "Crash" Animation Sequence
  const triggerGlitchSequence = () => {
    setIsGlitching(true);
    
    // After 3 seconds of chaos, show the Gate (Paywall Tease)
    setTimeout(() => {
      setShowGate(true);
    }, 3000);
  };

  // --- STATE 3: THE GATE (After Crash) ---
  if (showGate) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center font-mono">
        <motion.div 
          ...
          className="max-w-md w-full bg-[#0F172A] border border-blue-500/30 p-8 rounded-2xl shadow-2xl shadow-blue-500/20"
        >
          ...
          <div className="bg-black/50 p-4 rounded-lg border border-white/5 mb-6 text-left">
            ...
            <p className="text-white font-mono text-sm">Adaptive Algorithm Halt</p>
    ...
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center overflow-hidden">
        ...
        <motion.p 
          ...
          animate={{ opacity: 1 }}
          transition={{ delay: 1 }}
          className="mt-8 font-mono text-red-500 text-xl"
        >
          &gt; THETA_LIMIT_REACHED
          <br />
          &gt; TERMINATING SESSION...
    ...
    <div className="min-h-screen bg-[#F0F0F0] text-black font-sans flex flex-col">
      ...
      <header className="bg-[#2C3E50] text-white p-4 flex justify-between items-center shadow-md">
        <div>
          ...
          <p className="text-xs opacity-60">Candidate: Future Medic</p>
        </div>
        <div className="text-right">
          <p className="text-xl font-mono font-bold">01:58:42</p>
      ...
      <div className="w-full bg-gray-300 h-2">
        <div 
          ...
          style={{ width: `${(currentQIndex / 70) * 100}%` }}
        />
      </div>
      ...
      <main className="flex-1 max-w-4xl mx-auto w-full p-6 md:p-12 flex flex-col">
        <div className="flex-1">
          ...
          <h2 className="text-xl md:text-2xl font-medium text-gray-900 leading-relaxed mb-10">
            {question?.text || "Loading clinical scenario..."}
          </h2>
            ...
            {question?.options.map((option, idx) => (
              ...
              >
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                  selectedOption === idx ? "border-blue-600" : "border-gray-300"
                }`}>
          ...
          <button
            ...
            className={`px-8 py-3 rounded font-bold text-white transition-all ${
              selectedOption === null 
                ...
                : "bg-[#2C3E50] hover:bg-[#34495E] shadow-lg"
            }`}
          >
            NEXT QUESTION →
          </button>
